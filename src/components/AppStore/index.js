...
class AppStore extends Component {
  ...
  searchInput = event => {
    ...
    const enteredInput = event.target.value.toLowerCase()
  ...
  render() {
    ...
    const reFilteredProjects = filteredProjects.filter(each =>
      each.appName.toLowerCase().includes(userInput),
    )
    return (
      <div className="main-container">
        ...
        <ul className="tab-item-container">
          {tabsList.map(each => (
            <TabItem
              getActiveTabItem={this.getActiveTabItem}
              tabItemObj={each}
              key={each.tabId}
